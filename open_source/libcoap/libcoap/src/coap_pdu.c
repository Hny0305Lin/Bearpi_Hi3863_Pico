...
size_t coap_pdu_get_fix_header_size()
{
  return COAP_PDU_MAX_UDP_HEADER_SIZE;
}

size_t coap_pdu_parse_fix_header_size(coap_proto_t proto, const uint8_t *data, size_t length)
{
  assert(data);
  assert(proto == COAP_PROTO_TCP || proto == COAP_PROTO_TLS);
  assert(coap_pdu_get_fix_header_size() <= length);
...
coap_pdu_parse(coap_proto_t proto,
  ...
  if (length == 0)
    return 0;
#ifdef SUPPORT_OPTIC_ONT
  hdr_size = coap_pdu_get_fix_header_size();
#else
...
static size_t coap_pdu_encode_tcp_header(uint8_t e_token_length, size_t len, coap_pdu_t *pdu)
{
...
#ifdef SUPPORT_OPTIC_ONT
static size_t coap_pdu_encode_tcp_fix_header(uint8_t e_token_length, coap_pdu_t *pdu)
{
  if (pdu->max_hdr_size < 4) {
    coap_log(LOG_WARNING,
          "coap_pdu_encode_header: not enough space for TCP32 header\n");
  ...
  uint8_t tmp[2] = {0};
  uint16_t len1 = htons(pdu->used_size + 2);
  memcpy(tmp, &len1, sizeof(len1));
  pdu->token[-4] = tmp[0];
  pdu->token[-3] = tmp[1];
  pdu->token[-2] = (0x50 + e_token_length);
  pdu->token[-1] = pdu->code;
...
#ifdef SUPPORT_OPTIC_ONT
    pdu->hdr_size = coap_pdu_encode_tcp_fix_header(e_token_length, pdu);
#else
    pdu->hdr_size = coap_pdu_encode_tcp_header(e_token_length, len, pdu);
#endif
  } else {
...
size_t coap_pdu_get_used_size(const coap_pdu_t *pdu) {
  if (pdu == NULL) {
    return 0;
  }
...
size_t coap_pdu_get_max_size(const coap_pdu_t *pdu) {
  if (pdu == NULL) {
    return 0;
  }

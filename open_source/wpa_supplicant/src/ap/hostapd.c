...
#else
	conf->ssid.short_ssid = crc32_wpa(conf->ssid.ssid, conf->ssid.ssid_len);
#endif
#ifndef LOS_CONFIG_EXT_DRIVER_NOT_SUPPORT
	if (!hostapd_drv_none(hapd)) {
...
#ifdef LOS_WPA_PATCH
	wpa_comm_event_report(EXT_WIFI_EVT_AP_DISABLE, iface->bss[0]->msg_ctx, NULL);
#endif /* LOS_WPA_PATCH */
...
#ifdef LOS_WPA_PATCH
	wpa_comm_event_report(EXT_WIFI_EVT_AP_START, iface->bss[0]->msg_ctx, NULL);
#endif /* LOS_WPA_PATCH */
#ifdef CONFIG_P2P
	if (hapd->setup_complete_cb)
		hapd->setup_complete_cb(hapd->setup_complete_cb_ctx);
...
#ifdef LOS_WPA_PATCH
	wpa_comm_event_report(EXT_WIFI_EVT_AP_DISABLE, hapd->msg_ctx, NULL);
#endif /* LOS_WPA_PATCH */
...
#ifdef LOS_WPA_PATCH
	wpa_comm_event_report(EXT_WIFI_EVT_AP_DISABLE, hapd_iface->bss[0]->msg_ctx, NULL);
#endif /* LOS_WPA_PATCH */

...
#endif /* EXT_CODE_CROP */
#ifdef LOS_CONFIG_WPS_AP
	if (hapd->wps && hapd->wps->ap && hapd->wps->registrar) {
		wps_remove_device(hapd->wps->registrar, sta->addr);
	}
#endif
	...
	while (sta) {
		...
		if (sta->flags & WLAN_STA_AUTH) {
			...
			if (sta->p2p_ie)
				p2p_dev_addr = p2p_get_go_dev_addr(sta->p2p_ie);
			if (p2p_dev_addr != NULL)
				(void)hostapd_drv_sta_deauth(hapd, sta->addr, WLAN_REASON_DEAUTH_LEAVING);
#endif /* LOS_CONFIG_P2P */
		}
		sta = sta->next;
...
#else
		int fuzz = (int)(os_random() % 20);
		sta->timeout_next = STA_NULLFUNC;
		next_time = (unsigned long)(hapd->conf->ap_max_inactivity + fuzz);
#endif /* LOS_CONFIG_EXT_DRIVER_NOT_SUPPORT */
	}
			   ...
			   int authorized)
{
	...
	if (authorized) {
		...
		osal_printk("+NOTICE:STA CONNECTED\r\n");
#ifdef LOS_WPA_PATCH
		event_ap_sta_connected ap_sta_connected = {0};

		(void)os_memcpy(ap_sta_connected.addr, sta->addr, ETH_ALEN);
		(void)os_memcpy(ap_sta_connected.ifname, hapd->conf->iface, IFNAMSIZ);
		wpa_comm_event_report(EXT_WIFI_EVT_STA_CONNECTED, hapd->msg_ctx, &ap_sta_connected);
#endif /* LOS_WPA_PATCH */
#ifndef EXT_CODE_CROP
		...
		osal_printk("+NOTICE:STA DISCONNECTED\r\n");
#ifdef LOS_WPA_PATCH
		event_ap_sta_disconnected ap_sta_disconnected = {0};

		(void)os_memcpy(ap_sta_disconnected.addr, sta->addr, ETH_ALEN);
		(void)os_memcpy(ap_sta_disconnected.ifname, hapd->conf->iface, IFNAMSIZ);
		wpa_comm_event_report(EXT_WIFI_EVT_STA_DISCONNECTED, hapd->msg_ctx, &ap_sta_disconnected);
#endif /* LOS_WPA_PATCH */
#ifndef EXT_CODE_CROP
		if (hapd->msg_ctx_parent &&
		    hapd->msg_ctx_parent != hapd->msg_ctx)

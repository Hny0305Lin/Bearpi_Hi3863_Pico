...
skip_wpa_check:
...
#if defined(CONFIG_IEEE80211R_AP) || defined(CONFIG_FILS) || !defined(LOS_CONFIG_EXT_DRIVER_NOT_SUPPORT)
	...
	/* The CompanyName assoc frame response is implemented at the driver layer.
	   The following two scenarios need to be added during assoc request processing:
	   1. The AP sends the first eapol frame with the pmkid.
	      The code for the eapol frame to carry pmkid is added.
	   2. The AP mode is set to WPA2/WPA3. If the AP is associated with WPA3, change auth_alg to sae.
	      Disconnect the AP and associate it with WPA2 again.
	      If the AP does not receive a disconnect frame during the handover,
	      the value of auth_alg of the AP is sae during the association with WPA2 which lead the association fails.
	      Therefore, it need to change sta->auth_alg to open. */
...
#endif /* !defined(EXT_CODE_CROP) || defined(CONFIG_ACS) */
#ifdef HOSTAPD
#ifndef EXT_CODE_CROP
#ifdef NEED_AP_MLME
...
/* wpa_supplicant_event->hostapd_event */
#ifdef LOS_WPA_PATCH

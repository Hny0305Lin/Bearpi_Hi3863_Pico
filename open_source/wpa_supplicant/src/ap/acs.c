 ...
 */
#define ACS_PRINT_FACTOR 1000000
...
acs_survey_interference_factor(struct freq_survey *survey, s8 min_nf)
{
	...
	factor = pow(10, survey->nf / 5.0L) +
		((fabs(total) > FLT_EPSILON) ? (busy / total) : 0) *
		pow(2, pow(10, (float) survey->nf / 10.0L) -
		    pow(10, (float) min_nf / 10.0L));

	return factor;
}

static int acs_get_raw_data(float acs_data)
{
	int raw_acs_data;
	(void)memcpy_s(&raw_acs_data, sizeof(float), &acs_data, sizeof(float));
	return raw_acs_data;
}
				    ...
				    struct hostapd_channel_data *chan)
{
	...
	dl_list_for_each(survey, &chan->survey_list, struct freq_survey, list)
	{
		...
		osal_printk("ACS: %d: min_nf=%d interference_factor=0.%06d nf=%d time=%lu busy=%lu rx=%lu raw_factor[0x%X]\r\n",
			   i, chan->min_nf, (unsigned long)(int_factor * ACS_PRINT_FACTOR),
			   survey->nf, (unsigned long) survey->channel_time,
			   (unsigned long) survey->channel_time_busy,
			   (unsigned long) survey->channel_time_rx, acs_get_raw_data(int_factor));
	}
	...
	struct hostapd_iface *iface, struct hostapd_hw_modes *mode)
{
	...
	for (i = 0; i < mode->num_channels; i++) {
		...
		osal_printk("ACS:  * interference factor average: 0.%06d rawdata[0x%X]\r\n",
			   (unsigned long)(chan->interference_factor * ACS_PRINT_FACTOR), acs_get_raw_data(chan->interference_factor));
	}
}
			 ...
			 float *ideal_factor)
{
	...
	for (i = 0; i < mode->num_channels; i++) {
		...
		if (bias) {
			factor *= bias->bias;
			osal_printk("ACS:  * channel %d: total interference = 0.%06d (0.%06d bias) rawdata[0x%X]\r\n",
				   chan->chan, (unsigned long)(factor * ACS_PRINT_FACTOR), (unsigned long)(bias->bias * ACS_PRINT_FACTOR),
				   acs_get_raw_data(factor));
		} else {
			osal_printk("ACS:  * channel %d: total interference = 0.%06d rawdata[0x%X]\r\n",
				   chan->chan, (unsigned long)(factor * ACS_PRINT_FACTOR), acs_get_raw_data(factor));
		}
...
bw_selected:
	...
	if (ideal_chan) {
		osal_printk("ACS: Ideal channel is %d (%d MHz) with total interference factor of 0.%06d rawdata[0x%X]\r\n",
			   ideal_chan->chan, ideal_chan->freq, (unsigned long)(ideal_factor * ACS_PRINT_FACTOR),
			   acs_get_raw_data(ideal_factor));
		return ideal_chan;
	}

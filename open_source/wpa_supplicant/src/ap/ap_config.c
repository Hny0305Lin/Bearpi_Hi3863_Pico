...
void hostapd_config_defaults_bss(struct hostapd_bss_config *bss)
{
	...
	bss->ieee80211w = g_ap_opt_set.ieee80211w;
#ifdef CONFIG_HOSTAPD_WPA3
#ifndef CONFIG_SAE_NO_PW_ID
	if (g_ap_opt_set.sae_pwe <= WIFI_SAE_PWE_UNSPECIFIED) {
		bss->sae_pwe = WIFI_SAE_PWE_BOTH - 1;
	} else {
		bss->sae_pwe = g_ap_opt_set.sae_pwe - 1;
	}
#endif /* CONFIG_SAE_NO_PW_ID */
	bss->transition_disable = g_ap_opt_set.transition;
	if (g_ap_opt_set.sae_groups != NULL && g_ap_opt_set.sae_groups[0] != 0) {
		bss->sae_groups = g_ap_opt_set.sae_groups;
	}
...
#undef ecw2cw
	...
	if (g_ap_opt_set.beacon_period != 0)
		conf->beacon_int = (unsigned int)g_ap_opt_set.beacon_period;
#ifdef LOS_CONFIG_MESH
	if (g_mesh_flag)

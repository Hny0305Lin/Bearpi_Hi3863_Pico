...
static struct wps_registrar_device * wps_device_prev_get(struct wps_registrar *reg,
						    const struct wps_registrar_device *dst_dev)
{
	struct wps_registrar_device *dev = NULL;
	if (reg->devices == NULL) {
		return NULL;
	}
	for (dev = reg->devices; dev != NULL; dev = dev->next) {
		if (dev->next == dst_dev)
			return dev;
	}
	return NULL;
...
void wps_remove_device(struct wps_registrar *reg, const u8 *addr)
{
	struct wps_registrar_device *d = NULL;
	struct wps_registrar_device *prev = NULL;
	...
	if (reg->devices == d) {
		...
		prev = wps_device_prev_get(reg, d);
		if (prev != NULL)
			prev->next = d->next;
	}
	wps_device_data_free(&d->dev);
	os_free(d);
	return;
}
#endif /* LOS_CONFIG_WPS_AP */

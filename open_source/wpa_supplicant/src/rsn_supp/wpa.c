...
#if defined(CONFIG_WPA3) || defined(LOS_CONFIG_WPA_ENTERPRISE)
...
#endif /* defined(CONFIG_WPA3) || defined(LOS_CONFIG_WPA_ENTERPRISE) */
...
#if defined(CONFIG_WPA3) || defined(LOS_CONFIG_WPA_ENTERPRISE) || defined(CONFIG_IEEE80211R)
			       ...
			       struct wpa_ptk *ptk)
{
	...
	if ((sm->ctx != NULL) && (sm->ctx->ctx != NULL)) {
#ifndef CONFIG_ROAM_EXTRA_SUPPORT
		wpa_clear_passphrase(sm->ctx->ctx);
#endif
	}
	return ret;
}
					     ...
					     struct wpa_gtk_data *gd)
{
	...
	if (ieee80211w_set_keys(sm, &ie) < 0)
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
			"RSN: Failed to configure IGTK");
#else
	(void)ieee80211w_set_keys(sm, &ie);
#endif /* EXT_CODE_CROP */
	return 0;
}

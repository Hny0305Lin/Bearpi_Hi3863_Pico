...
int g_ssid_reconn_count = 1;
int g_ssid_reconn_max_count = 0;
struct wpa_bss **g_conf_aps_bssid = NULL;
#endif

#ifdef LOS_WPA_PATCH
void wpa_comm_event_report(ext_wifi_event_type event, const void *ctx, const void *data)
{
	ext_wifi_event wifi_events = {0};
	struct ext_wifi_dev *wifi_dev = NULL;

	wifi_dev = los_get_wifi_dev_by_priv(ctx);
	if (wifi_dev == NULL)
		return;

	wifi_events.event = event;
	switch(wifi_events.event) {
		case EXT_WIFI_EVT_SCAN_DONE:
		case EXT_WIFI_EVT_MESH_SCAN_DONE:
		case EXT_WIFI_EVT_MESH_STA_SCAN_DONE: {
			if (data == NULL)
				return;
			wifi_events.info.wifi_scan_done.bss_num = *(int *)data;
			(void)os_memcpy(wifi_events.info.wifi_scan_done.ifname, wifi_dev->ifname, os_strlen(wifi_dev->ifname) + 1);
			break;
		}
		case EXT_WIFI_EVT_CONNECTED:
			if (data == NULL)
				return;
			(void)os_memcpy(&wifi_events.info.wifi_connected, data, sizeof(wifi_events.info.wifi_connected));
			break;
		case EXT_WIFI_EVT_DISCONNECTED:
			if (data == NULL)
				return;
			(void)os_memcpy(&wifi_events.info.wifi_disconnected, data, sizeof(wifi_events.info.wifi_disconnected));
			break;
		case EXT_WIFI_EVT_STA_CONNECTED:
			if (data == NULL)
				return;
			(void)os_memcpy(&wifi_events.info.ap_sta_connected, data, sizeof(wifi_events.info.ap_sta_connected));
			break;
		case EXT_WIFI_EVT_STA_DISCONNECTED:
			if (data == NULL)
				return;
			(void)os_memcpy(&wifi_events.info.ap_sta_disconnected, data, sizeof(wifi_events.info.ap_sta_disconnected));
			break;
		case EXT_WIFI_EVT_STA_NO_NETWORK:
			(void)os_memcpy(wifi_events.info.scan_no_network.ifname, wifi_dev->ifname, os_strlen(wifi_dev->ifname) + 1);
			break;
		case EXT_WIFI_EVT_AP_START:
			(void)os_memcpy(wifi_events.info.wifi_ap_start.ifname, wifi_dev->ifname, os_strlen(wifi_dev->ifname) + 1);
			break;
		case EXT_WIFI_EVT_AP_DISABLE:
			(void)os_memcpy(wifi_events.info.wifi_ap_fail.ifname, wifi_dev->ifname, os_strlen(wifi_dev->ifname) + 1);
			break;
		default:
			return;
	}

	wifi_new_task_event_cb(&wifi_events);
}
#endif /* LOS_WPA_PATCH */
...
static void wpa_supplicant_update_current_bss(struct wpa_supplicant *wpa_s)
{
	...
	if (bss) {
        if ((wpa_s->current_bss != NULL) && (wpa_s->current_bss != bss)) {
            os_free_drv_scan_bss(wpa_s->current_bss);
...
void wpa_supplicant_mark_disassoc(struct wpa_supplicant *wpa_s)
{
	...
	if ((wpa_is_sta(wpa_s) == WPA_FLAG_OFF) || (g_reconnect_set.pending_flag == WPA_FLAG_OFF))
...
#else
	wep_ok = !wpa_key_mgmt_wpa(ssid->key_mgmt) &&
		((((unsigned int)ssid->key_mgmt & WPA_KEY_MGMT_NONE) &&
		  ssid->wep_key_len[ssid->wep_tx_keyidx] > 0));
#endif /* EXT_WPA_KEY_MGMT_CROP */
#endif /* CONFIG_WEP */
...
#ifdef CONFIG_WAPI
	const u8 *wapi_ie = wpa_bss_get_ie(bss, WLAN_EID_WAPI);
	wpa_dbg(wpa_s, MSG_DEBUG, "ssid->proto is %d, WPA_PROTO_WAPI is %d", ssid->proto, WPA_PROTO_WAPI);
			    ...
			    bool debug_print)
{
	...
	if (ie && wpa_parse_wpa_ie_rsn(ie, 2 + ie[1], &data) == 0 &&
        ...
        /* 在无wpa/wpa2时保证进入wpa_supplicant_ssid_bss_match*/
	...
	if (!osen && !wpa &&
		...
		!((unsigned int)ssid->key_mgmt & WPA_KEY_MGMT_NONE) &&
		!(ssid->key_mgmt & WPA_KEY_MGMT_OWE) &&
		!((unsigned int)ssid->key_mgmt & WPA_KEY_MGMT_WPS)) {
#endif /* EXT_WPA_KEY_MGMT_CROP */
		if (debug_print)
			wpa_dbg(wpa_s, MSG_DEBUG,
...
__attribute__((weak)) struct wpa_bss *wpa_supplicant_select_bss(struct wpa_supplicant *wpa_s,
			  struct wpa_ssid *group,
...
#endif /* EXT_CODE_CROP */
#ifndef LOS_WPA_PATCH
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group, only_first_ssid, 0);
#else
...
__attribute__((weak)) struct wpa_bss * wpa_supplicant_pick_network(struct wpa_supplicant *wpa_s,
					     struct wpa_ssid **selected_ssid)
{
					      ...
					      int own_request, int update_only)
{
	...
	if ((wifi_dev != NULL) &&
		((wifi_dev->iftype == EXT_WIFI_IFTYPE_STATION) ||
		(wifi_dev->iftype == EXT_WIFI_IFTYPE_MESH_POINT) ||
		((wifi_dev->iftype >= EXT_WIFI_IFTYPE_P2P_CLIENT) &&
		(wifi_dev->iftype <= EXT_WIFI_IFTYPE_P2P_DEVICE)))) {
		uapi_at_printf("+NOTICE:SCANFINISH\r\n");
	}
#ifdef LOS_WPA_PATCH
	int num = 0;
	...
	if (g_scan_flag == WPA_FLAG_ON) {
		if (wpa_s->ifmsh != NULL)
			event = EXT_WIFI_EVT_MESH_SCAN_DONE;
		else
			event = g_mesh_sta_flag ? EXT_WIFI_EVT_MESH_STA_SCAN_DONE : EXT_WIFI_EVT_SCAN_DONE;
		g_scan_flag = WPA_FLAG_OFF;

		if (scan_res != NULL)
			wpa_comm_event_report(event, wpa_s, &(scan_res->num));
		else
			wpa_comm_event_report(event, wpa_s, &num);
	}
	if ((wifi_dev->iftype >= EXT_WIFI_IFTYPE_P2P_CLIENT) &&
		(wifi_dev->iftype <= EXT_WIFI_IFTYPE_P2P_DEVICE)) {
		if (scan_res != NULL)
			wpa_comm_event_report(EXT_WIFI_EVT_SCAN_DONE, wpa_s, &(scan_res->num));
		else
			wpa_comm_event_report(EXT_WIFI_EVT_SCAN_DONE, wpa_s, &num);
	}
#endif /* LOS_WPA_PATCH */
	if (scan_res == NULL) {
#ifndef EXT_CODE_CROP
...
scan_no_result:
	...
	if (wpa_s->p2p_scan_work) {
		...
		radio_work_done(work);
		p2p_scan_res_handled(wpa_s->global->p2p, 0);
	}
#endif
	...
	if (selected) {
...
#ifdef CONFIG_MESH
		if (wpa_s->ifmsh != NULL) {
			if (los_meshap_is_connecting())
				los_wpa_meshap_req_scan(wpa_s, WPA_MESH_PEER_SCAN_TIMEOUT, 0);
			return 0;
		}
#endif /* CONFIG_MESH */
		if (wpa_supplicant_enabled_networks(wpa_s)) {
...
#endif
			uapi_at_printf("+NOTICE:NETWORK NOT FIND\r\n");
#ifdef LOS_WPA_PATCH
			wpa_comm_event_report(EXT_WIFI_EVT_STA_NO_NETWORK, wpa_s, NULL);
#endif /* LOS_WPA_PATCH */
		}
		...
		if (wpa_is_sta(wpa_s) == WPA_FLAG_ON) {
			if ((g_fast_connect_flag == WPA_FLAG_ON) && (g_fast_connect_scan_flag == WPA_FLAG_ON)) {
				...
				(void)os_event_write(g_wpa_event, WPA_EVENT_QUICK_CONNECT_ERROR);
				return -1;
			}
...
__attribute__((weak)) int wpa_supplicant_fast_associate(struct wpa_supplicant *wpa_s)
{
					      ...
					      union wpa_event_data *data)
{
...
#ifdef CONFIG_ROAM_EXTRA_SUPPORT
	/*
	...
	if (wpa_s->key_mgmt != ie.key_mgmt &&
		...
		if (wpa_s->conf->key_mgmt_offload &&
		    ...
		    wpa_drv_set_key(wpa_s, 0, NULL, 0, 0, NULL, 0,
					  ...
					  union wpa_event_data *data)
{
	...
	const u8 *p;
	u8 bssid[ETH_ALEN];
	bool bssid_known = wpa_drv_get_bssid(wpa_s, bssid) == 0;
    bool ret = wpa_drv_get_bssid(wpa_s, bssid);
...
#ifdef CONFIG_WAPI
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK) {
		wapi_asue_event(wpa_s, EVENT_ASSOC, data);
...
#ifdef CONFIG_WAPI
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK)
		wapi_asue_event(wpa_s, EVENT_DISASSOC, NULL);
#endif /* CONFIG_WAPI */
...
{
#ifdef LOS_WPA_PATCH
	/* wpa3 场景密钥在auth就会校验, 失败时驱动会上报WLAN_MAC_EXT_AUTH_FAIL + status的错误码;
	...
	if (wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) && wpa_s->wpa_state == WPA_ASSOCIATED &&
...
#ifdef CONFIG_SSID_RECONNECT
static void wpa_supplicatn_ssid_reconnect(void *eloop_ctx, void *timeout_ctx)
{
	(void)timeout_ctx;
	if (g_conf_aps_bssid == NULL || eloop_ctx == NULL)
		return;
	struct wpa_supplicant *wpa_s = eloop_ctx;
	struct wpa_bss *last_bss = *(g_conf_aps_bssid + g_ssid_reconn_count - 1);
	wpa_supplicant_cancel_scan(wpa_s);
	if (wpa_s->wpa_state >= WPA_AUTHENTICATING)
		wpas_connection_failed(wpa_s, last_bss->bssid);
	if (g_ssid_reconn_count < g_ssid_reconn_max_count) {
		struct wpa_bss *bss = *(g_conf_aps_bssid + g_ssid_reconn_count);
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		g_ssid_reconn_count++;
		if (ssid != NULL && bss != NULL && wpa_supplicant_connect(wpa_s, bss, ssid) < 0)
			wpa_error_log0(MSG_DEBUG, "Auto ssid_reconnect failed!");
	}
}
#endif
						 ...
						 int locally_generated)
{
...
#ifdef CONFIG_SSID_RECONNECT
	if (g_ssid_reconn_count < g_ssid_reconn_max_count) {
		(void)eloop_register_timeout(0, 5, wpa_supplicatn_ssid_reconnect, wpa_s, NULL); /* 5ms timeout */
		return;
	}
#endif
	...
	if (wpa_s->wpa_state >= WPA_AUTHENTICATING) {
		...
		if (g_sta_delay_report_flag == WPA_FLAG_OFF)
			uapi_at_printf("+NOTICE:DISCONNECTED\r\n");
#ifndef EXT_CODE_CROP
		else
		...
		if ((g_wpa_event_cb != NULL) && (g_sta_delay_report_flag == WPA_FLAG_OFF)) {
			event_wifi_disconnected disconnect = {0};

			(void)os_memcpy(disconnect.ifname, wpa_s->ifname, WIFI_IFNAME_MAX_SIZE);
			(void)os_memcpy(disconnect.bssid, bssid, ETH_ALEN);
			disconnect.reason_code = reason_code;
			disconnect.locally_generated = locally_generated;
			disconnect.wpa_state = wpa_s->wpa_state;
			/* 
				可能密码错误的流程有2种: wpa3:auth; 其他加密:eapol
				1) eapol流程密码错误有2种情况
					a) 对端校验失败主动发deauth踢, 此时直接上报
					b) 由于对端无响应, sta主动发deauth断, reason为3, 此时将reason转为15上报给应用
				2) wpa3 auth流程密码错误
				wpa3密码在auth校验, 驱动校验失败时会上报失败status + WLAN_MAC_EXT_AUTH_FAIL的reason, 此时转为15报给应用
			*/
			if (could_be_psk_mismatch(wpa_s, reason_code, locally_generated) &&
				((locally_generated == 1 && reason_code == WLAN_REASON_DEAUTH_LEAVING) || reason_code >= WLAN_MAC_EXT_AUTH_FAIL)) {
...
#endif /* CONFIG_MESH */
#ifdef CONFIG_SDP
	wpa_dbg(wpa_s, MSG_DEBUG, "sdp_rx_action input .");
	if (category == WLAN_ACTION_PUBLIC &&
		payload[0] == WLAN_PA_VENDOR_SPECIFIC &&
		WPA_GET_BE24(&payload[1]) == OUI_WFA &&
		payload[4] == SDP_OUI_TYPE) {  /* 4: SDP payload */
		sdp_rx_action(wpa_s, category, payload, plen);
		return;
	}
#endif /* CONFIG_SDP */
}
			  ...
			  union wpa_event_data *data)
{
...
#ifdef CONFIG_MESH
	if ((wifi_dev->iftype == EXT_WIFI_IFTYPE_MESH_POINT) && (wpa_sta_event_check(event) == 0)) {
		hostapd_event(g_hapd, event, data);
		return;
	}
#endif /* CONFIG_MESH */
	...
	case EVENT_DISASSOC:
		...
		if (g_wpa_rm_network == SOC_WPA_RM_NETWORK_WORKING)
	...
	case EVENT_CH_SWITCH:
		...
		if (!data || !wpa_s->current_ssid)
			break;
#ifdef LOS_WPA_PATCH
		if (los_freq_to_channel(data->ch_switch.freq, &channel) != EXT_WIFI_OK)
			break;
#endif /* LOS_WPA_PATCH */
#ifndef EXT_CODE_CROP
		wpa_msg(wpa_s, MSG_INFO,
	...
	case EVENT_NEW_PEER_CANDIDATE:
		...
		if (g_mesh_auto_peer_flag) {
			los_event_new_mesh_peer(wpa_s, data->ext_mesh_peer.peer,
				data->ext_mesh_peer.mesh_bcn_priority, data->ext_mesh_peer.mesh_is_mbr);
		}
#endif /* CONFIG_MESH */
		break;

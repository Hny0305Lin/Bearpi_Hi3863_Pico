...
#ifdef LOS_WPA_PATCH
		if (wpa_get_scan_triggered_by_rm_net_cli_flag()) {
			wpa_set_scan_triggered_by_rm_net_cli_flag(false);
...
#endif /* EXT_CODE_CROP */
#ifdef LOS_WPA_PATCH
	if ((wpa_is_sta(wpa_s) == WPA_FLAG_ON) && (!g_usr_scanning_flag) && g_connecting_flag &&
...
void wpa_supplicant_cancel_scan(struct wpa_supplicant *wpa_s)
{
	...
	if ((wpa_s != NULL) && (wpa_s->ifmsh != NULL))
		eloop_cancel_timeout(los_wpa_mesh_scan, wpa_s, NULL);
 ...
 */
__attribute__((weak)) struct wpa_scan_results *
		       ...
		       struct wpa_scan_results *scan_res)
{
	...
	if ((wifi_dev != NULL) &&
		...
		osal_printk("+NOTICE:SCANFINISH\r\n");
	}

	wpa_comm_event_report(EXT_WIFI_EVT_SCAN_DONE, wpa_s, NULL)
#endif /* LOS_WPA_PATCH */
...
#endif /* EXT_CODE_CROP */
#ifdef CONFIG_ACS
	params->acs_scan_flag = src->acs_scan_flag;
#endif

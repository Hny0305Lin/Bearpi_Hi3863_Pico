...
static inline int wpa_drv_handle_app_service(struct wpa_supplicant *wpa_s, void *params)
{
    if (wpa_s->driver->wifi_app_service_handle)
        return wpa_s->driver->wifi_app_service_handle(wpa_s->drv_priv, params);
    return -1;
}

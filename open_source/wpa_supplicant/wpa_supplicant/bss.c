 ...
 */
__attribute__((weak)) struct wpa_bss * wpa_bss_get(struct wpa_supplicant *wpa_s, const u8 *bssid,
				    ...
				    struct os_reltime *fetch_time)
{
	...
	(void)os_memcpy(bss->ies, res + 1, res->ie_len + res->beacon_ie_len);
#else
	(void)os_memcpy(bss->ies, res + 1, res->ie_len);
#endif /* EXT_SCAN_SIZE_CROP */
			     ...
			     struct os_reltime *fetch_time)
{
...
#ifndef CONFIG_DRIVER_SOC
	/* CompanyName driver do not support mesh id IEs */
	...
	if ((g_mesh_flag && (wpa_s->ifmsh != NULL)) ||
		...
		mesh = los_wpa_scan_get_mesh_id(res);
		if (mesh && (mesh[1] <= SSID_MAX_LEN))
			ssid = mesh;
	}
#endif /* CONFIG_MESH */
 ...
 * Returns: Pointer to the BSS entry or %NULL if not found
 */
__attribute__((weak)) struct wpa_bss * wpa_bss_get_bssid(struct wpa_supplicant *wpa_s,
				   const u8 *bssid)
{
	struct wpa_bss *bss;
#ifndef EXT_CODE_CROP
 ...
 */
__attribute__((weak)) struct wpa_bss * wpa_bss_get_bssid_latest(struct wpa_supplicant *wpa_s,
					  const u8 *bssid)
{

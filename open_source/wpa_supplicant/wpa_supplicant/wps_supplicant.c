...
#ifdef LOS_WPA_PATCH
void wpa_wps_event_report(ext_wifi_event_type event, const void *ctx, void *data)
{
	...
	switch(wifi_events.event) {
		case EXT_WIFI_EVT_WPS_TIMEOUT:
			(void)os_memcpy(wifi_events.info.wifi_wps_timeout.ifname, wifi_dev->ifname, os_strlen(wifi_dev->ifname) + 1);
			break;
		case EXT_WIFI_EVT_WPS_SUCCESS:
			(void)os_memcpy(wifi_events.info.wifi_wps_success.ifname, wifi_dev->ifname, os_strlen(wifi_dev->ifname) + 1);
			break;
		case EXT_WIFI_EVT_WPS_FAIL: {
			struct wps_event_fail *fail;
			if (data == NULL)
				return;
			(void)os_memcpy(wifi_events.info.wifi_wps_fail.ifname, wifi_dev->ifname, os_strlen(wifi_dev->ifname) + 1);
			fail = (struct wps_event_fail *)data;
			wifi_events.info.wifi_wps_fail.msg = fail->msg;
			wifi_events.info.wifi_wps_fail.config_error = fail->config_error;
			wifi_events.info.wifi_wps_fail.error_indication = fail->error_indication;
			break;
		}
		default:
			return;
...
#ifdef LOS_WPA_PATCH
		wpa_wps_event_report(EXT_WIFI_EVT_WPS_FAIL, wpa_s, NULL);
#endif /* LOS_WPA_PATCH */
	}
...
#ifdef LOS_WPA_PATCH
		wpa_wps_event_report(EXT_WIFI_EVT_WPS_FAIL, wpa_s, fail);
#endif /* LOS_WPA_PATCH */
#ifdef CONFIG_P2P
		if (wpa_s->p2pdev && wpa_s->p2pdev != wpa_s) {
			wpa_msg(wpa_s->p2pdev, MSG_INFO, WPS_EVENT_FAIL
				...
				wps_ei_str(fail->error_indication));
#ifdef LOS_WPA_PATCH
			wpa_wps_event_report(EXT_WIFI_EVT_WPS_FAIL, wpa_s->p2pdev, fail);
#endif /* LOS_WPA_PATCH */
		}
#endif /* CONFIG_P2P */
	} else {
...
#ifdef LOS_WPA_PATCH
	wpa_wps_event_report(EXT_WIFI_EVT_WPS_SUCCESS, wpa_s, NULL);
#endif /* LOS_WPA_PATCH */
	wpa_s->wps_success = 1;
#ifndef EXT_CODE_CROP
...
#ifdef LOS_WPA_PATCH
	wpa_wps_event_report(EXT_WIFI_EVT_WPS_TIMEOUT, wpa_s, NULL);
#endif /* LOS_WPA_PATCH */
...
__attribute__((weak)) struct wpa_ssid * wpas_wps_add_network(struct wpa_supplicant *wpa_s,
...
__attribute__((weak)) void wpas_wps_update_ap_info(struct wpa_supplicant *wpa_s,

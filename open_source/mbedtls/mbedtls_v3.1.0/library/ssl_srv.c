...
#if defined(VENDOR_ONT_TLS_CERT_FAILED_NOTICE_C)
...
int mbedtls_ssl_set_client_transport_id( mbedtls_ssl_context *ssl,
                                 ...
                                 size_t ilen )
{
#if defined(VENDOR_IOT_DTLS_CLIENT_SET_COOKIE_C)
    if( ssl->conf->endpoint == MBEDTLS_SSL_IS_CLIENT )
    {
        if( ssl->handshake->verify_cookie != NULL )
            mbedtls_free( ssl->handshake->verify_cookie );
 
        ssl->handshake->verify_cookie = mbedtls_calloc( 1, ilen );
        if( ssl->handshake->verify_cookie == NULL )
            return( MBEDTLS_ERR_SSL_ALLOC_FAILED );
 
        memcpy( ssl->handshake->verify_cookie, info, ilen );
        ssl->handshake->verify_cookie_len = ilen;
        return( 0 );
    }
#endif
...
#if !defined(VENDOR_ONT_CUSTOMIZE_SSL_CIPHERSUIE_MATCH_C)
        return( 0 );
#endif
    }
#endif
...
read_record_header:
...
#endif
    {
        ...
        if( ( ret = mbedtls_ssl_fetch_input_expand( ssl, 5 , 0)) != 0 )
#else
...
#if defined(VENDOR_ONT_CUSTOMIZE_SSL_CIPHERSUIE_MATCH_C)
            for( curve = ssl->handshake->curves; (curve != NULL && *curve != NULL); curve++ )
#else

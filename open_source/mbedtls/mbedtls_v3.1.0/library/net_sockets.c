 ...
 */
#ifdef ESCP_MBDTLS_CUSTOMIZATION
int mbedtls_net_connect_internel( mbedtls_net_context *ctx, const char *host,
                         const char *port, int proto, int blockConnect )
{
    ...
    for( cur = addr_list; cur != NULL; cur = cur->ai_next )
    {
        ...
        if(FALSE == blockConnect) { //NONBLOCK
            flags = fcntl(ctx->fd , F_GETFL, 0);
            (VOS_VOID)fcntl(ctx->fd , F_SETFL, flags | O_NONBLOCK);
...
{
    return mbedtls_net_connect_internel(ctx, host, port, proto, 1);
}
...
#ifndef ESCP_MBDTLS_CUSTOMIZATION
        else
        {

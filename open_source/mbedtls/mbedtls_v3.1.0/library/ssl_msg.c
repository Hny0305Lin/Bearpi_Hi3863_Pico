...
int mbedtls_ssl_check_header(mbedtls_ssl_context const *ssl)
{
    if (( ssl_check_record_type( ssl->in_hdr[0] ) ) || // 0类型校验
       ( ssl->in_hdr[1] != ssl->major_ver ) ||         // 1大版本号校验
       ( ssl->in_hdr[2] > ssl->conf->max_minor_ver ))  // 2小版本号校验
    {
        TRACE_VISUAL("format:%2x %02x %02x", ssl->in_hdr[0], ssl->in_hdr[1], ssl->in_hdr[2]); // 0类型 1大版本号 2小版本号
        return( MBEDTLS_ERR_SSL_INVALID_RECORD );
    }
    return 0;
}
...
int mbedtls_ssl_fetch_input_expand( mbedtls_ssl_context *ssl, size_t nb_want, int retry)
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t len;
    int waite = 5000;  // 超时等待时间5000ms
    struct timeval tv;
...
{
    return mbedtls_ssl_fetch_input_expand(ssl, nb_want, 1);
}
...
#ifdef ESCP_MBDTLS_CUSTOMIZATION
    ret = mbedtls_ssl_fetch_input_expand( ssl, mbedtls_ssl_in_hdr_len( ssl ), 0 );
#else

 ...
 */
typedef struct {
    unsigned char mac[WIFI_ANY_MAC_LEN];  /**<the MAC address for peer device. *//**<CNcomment: 收发操作的对端MAC地址 */
    unsigned char channel;                /**<the channel number of peer device,the value 0xFF means current channel. */
...
* @param  devices [IN]  Type  #ext_wifi_any_device *, array of poniter of ANY devices found.CNcomment:发现的ANY设备信息,
*                       该参数为指针类型的数组。CNend
...
* @par Dependency:
*            @li soc_any_api.h: ANY API
* @see  uapi_wifi_any_discover_peer
*/
...
* @par Dependency:
*            @li soc_any_api.h: ANY API
* @see  uapi_wifi_any_set_callback
*/
typedef void (*uapi_wifi_any_recv_cb)(unsigned char *mac, unsigned char *data, unsigned short len,
 ...
 */
typedef struct {
    uapi_wifi_any_send_complete_cb send_cb; /**<the callback function invoked when an ANY frame is sent. */
                                          /**<CNcomment: ANY发送完成驱动回调函数，用于将驱动发送结果反馈给上层 */
    uapi_wifi_any_recv_cb recv_cb;          /**<the callback function invoked when an ANY frame is received. */
                                          /**<CNcomment: ANY接收数据驱动回调函数，用于将驱动接收的数据传递给上层 */
} ext_wifi_any_callback;
...
* @param  send_cb  [IN]  Type  #uapi_wifi_any_send_complete_cb, callback function for ANY TX.
*                              CNcomment:ANY驱动发送回调函数.CNend
* @param  recv_cb  [IN]  Type  #uapi_wifi_any_recv_cb, callback function for ANY RX.
*                              CNcomment:ANY驱动接收回调函数.CNend
*
* @retval #void    no return value.CNcomment:无返回值.CNend
* @par Dependency:
*            @li soc_any_api.h: ANY API
* @see  uapi_wifi_any_send_complete_cb | uapi_wifi_any_recv_cb
*/
...
* @par Dependency:
*            @li soc_any_api.h: ANY API
* @see  uapi_wifi_any_send_complete_cb
*/
int uapi_wifi_any_send(const unsigned char *mac, unsigned char mac_len, unsigned char *data, unsigned short data_len,
    unsigned char seq);
...
* @param  peer_info       [IN]     Type  #ext_wifi_any_peer_info *, information of peer device.
...
* @param  peer            [OUT]    Type  #ext_wifi_any_peer_info *, peer device's information.

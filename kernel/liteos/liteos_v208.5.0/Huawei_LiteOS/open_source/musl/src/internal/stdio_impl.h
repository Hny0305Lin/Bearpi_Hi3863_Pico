...
#ifdef __LITEOS__
#define FLOCK_ARG(l, f) int __need_unlock = (((((l) != (void *)0)) && ((f)->lock >= 0)) ? (l)->lock((f)) : 0)
#define FUNLOCK_ARG(l, f) do { if (__need_unlock) (l)->unlock((f)); } while (0)
#endif
...
struct lockfile_func {
	int (*lock)(FILE *);
	void (*unlock)(FILE *);
};
hidden int vfscanf_unlocked(FILE *restrict f, const char *restrict fmt, va_list ap);
hidden int vfprintf_unlocked(FILE *restrict f, const char *restrict fmt, va_list ap);
#endif
